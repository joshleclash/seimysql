<?php
	/**
	 * Esta función se encarga de mostrar los mapas conceptuales que el estudiante aun no ha jugado
	...
	function recordatorioEstudiante(){
		...
		if(pg_num_rows($QueryRec)==0){
			$Salida.= "<tr><th>No hay recordatorios.</th></tr>";
		}
		else{
			while($VecMap=pg_fetch_array($QueryRec)){
				...
				if(pg_num_rows($QueryVerif)<2){
					...
					if(pg_num_rows($QueryVerif)==0){
						$SqlJuego = "SELECT nombre_juego FROM juego;";
						$QueryJuego = pg_query($SqlJuego);
		
						if(!$QueryJuego){
							$Respuesta->AddAlert("No se ha podido traer datos de la BD.");
							return $Respuesta->getXML();
						}
						while($NomJuego=pg_fetch_array($QueryJuego)){
							$Salida.= $NomJuego[0]."<br>";
						}
					}
					else{
						while($VecJuego=pg_fetch_array($QueryVerif)){
	...
	/**
	 * Esta función se encarga de mostrar el score del estudiante para todos los grupos a los que pertenece con un porcentaje que determina su posicion en la tabla
	 * @return xajaxResponse Objeto con la respuesta de la libreria XAjax
	 */
	function rankingEstudiante(){
		...
		if(pg_num_rows($QueryJuego)==0){
			...
			$Salida.= "<tr align='right'><td colspan='".pg_num_rows($QueryJuego)."'><a href='javascript:;' onClick='xajax_recordatorioEstudiante();'>Ver Recordatorios</a></td></tr>";
			$Salida.= "<tr><th colspan='".pg_num_rows($QueryJuego)."'>TABLAS RANKING POR JUEGO<br><br></th></tr>";
			$Salida.= "<tr><td colspan='".pg_num_rows($QueryJuego)."' align='justify'>A continuaci&oacute;n se muestran las tablas de ranking por juego para todos los mapas conceptuales del(os) grupo(s) del estudiante.<br><br></td></tr>";
			...
			while($VecJuego=pg_fetch_array($QueryJuego)){
				...
				$QueryEst = pg_query($SqlEst);
				if(!$QueryEst){
					$Respuesta->AddAlert("No se ha podido traer datos de la BD.");
					return $Respuesta->getXML();
				}
				if(pg_num_rows($QueryEst)==0){
					...
					while($VecEst=pg_fetch_array($QueryEst)){
						...
						$QueryRank = pg_query($SqlRank);
						if(!$QueryRank){
							$Respuesta->AddAlert("No se ha podido traer datos de la BD.");
							return $Respuesta->getXML();
						}
						if(pg_num_rows($QueryRank)==0){
							$Vacios[$Pos1]["val"] = 0;
							...
							$Acumulado = 0;
							$Duracion = 0;
							while($VecRank=pg_fetch_array($QueryRank)){
								$VecResp = explode("/",$VecRank[0]);
								$Acumulado+= $VecResp[0] / $VecResp[1];
								$Duracion+= $VecRank[1];
							}
							$Valoracion = ($Acumulado / pg_num_rows($QueryRank))*100;
							$Duracion = $Duracion / pg_num_rows($QueryRank);
							$Rank[$Pos2]["val"] = round($Valoracion,2);
							$Rank[$Pos2]["nom"] = $VecEst[2]." ".$VecEst[1];
							$Rank[$Pos2]["map"] = pg_num_rows($QueryRank);
							if($VecEst[0]==$_SESSION["NumIdentidad"]){
