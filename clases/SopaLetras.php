<?php
	...
	*/
		...
		while($Vec=pg_fetch_array($QueryMapa)){
			...
			if(pg_num_rows($QueryDoc)>0){
				while($VecD=pg_fetch_array($QueryDoc)){
					...
					$FechaLimite=$VecD[3];
					if($EstadoMapa=="1"){
						$Juego=pg_fetch_assoc(pg_query("SELECT id_juego FROM juego WHERE nombre_juego='Sopa Letras'"));
	...
	/**
	* Esta función se encarga de mostrar la sopa de letras
	...
	*/
	function generarSopaLetras($IdMapa)
	{
		...
		if($Conexion!=false)
		{
			...
			{
				$cont=1;
				while($VecResp=pg_fetch_assoc($ResSelect))
				{
				...
				//return $Respuesta;
				$ResSelectInc=pg_query($QueryInc);
				if($ResSelectInc){
					if(pg_num_rows($ResSelectInc)>0){
						$mayorinc=0;
						while($VecInc=pg_fetch_assoc($ResSelectInc)){	
				...
				$VecJuego=pg_fetch_assoc(pg_query($SqlTiempoLim));
	...
	/**
	* Esta función se encarga de preparar la sopa de letras
	* @param string[] $valores Vector que contiene las palabras de la sopa de letras
	* @param integer $IdMap Código del mapa
	* @return string $Html Cadena que contiene todo el código HTML de la sopa de letras
	*/
	function mostrarSopaLetras($valores,$IdMap)
	{
		  	ob_start();
			//aqui arranca la organizacion de la sopa de letras
				$abecedario = array("a","b","c","d","e","f","g","h","i","j","k","l","m","n","ñ","o","p","q","r","s","t","u","v","w","x","y","z","1","2","3","4","5","6","7","8","9","0","&aacute;","&eacute;","&iacute;","&oacute;","&uacute;");
				$Matriz = array();
				$hor=rand(0,25);
				$ver=rand(0,25);
