	...
	if(trim(ClaveUno.value)==""){
		...
		if(trim(ClaveDos.value)==""){
			...
			if(ClaveUno.value!=ClaveDos.value){
				alert("La clave de confirmaci�n no coincide con la digitada. Por favor, verifique e intente de nuevo.");
				ClaveUno.value="";
				ClaveDos.value="";
				ClaveUno.focus();
			}
...
function validarFormaMapa(Forma){
	...
	if(Forma.IdTipoMapa.value==""){
		...
		if(Forma.TematicaMapa.value=="0"){
			alert("Por favor, seleccione un tem�tica para subir.");	
		}
		else
		{
...
function confirmarActivacionMapa(id,estado,letrero){
	if(letrero==1){
		textoletrero="\n\nADVERTENCIA:\nSi hace click en Aceptar, NO PODR� VOLVER A EDITAR EL MAPA CONCEPTUAL.\nSi a�n desea hacer cambios en el mapa, haga click en Cancelar.";
	}
	else{
		textoletrero="";
	}
	if(confirm("�Est� seguro de querer activar el mapa?"+textoletrero)==true){
		if(letrero!=1){
