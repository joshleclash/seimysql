<?php
/**
 * Esta funci�n se encarga de devolver el nombre del mes
 * @param string $Num N�mero del mes
...
/**
 * Esta funci�n se encarga de quitar el contenido HTML de los campos especificados
...
/**
 * Esta funci�n se encarga de mostrar una lista desplegable con los grupos del docente
 * @param integer $NumDocente N�mero de identificaci�n del docente
 * @param integer $TipoGrafica C�digo de la gr�fica seleccionada
 * @return xajaxResponse Objeto con la respuesta de la libreria XAjax
 */
function cargarGruposGrafica($NumDocente,$TipoGrafica){
	...
	$QueryGrupos=mysql_query("SELECT id_grupo as id, nombre_grupo as nom FROM grupo WHERE id_grupo IN (SELECT grupo_id_grupo FROM grupo_usuario WHERE usuario_id_usuario='".$NumDocente."') order by id");
	$NumGrupos=mysql_num_rows($QueryGrupos);
	if($NumGrupos>0){
...
/**
 * Esta funci�n se encarga de mostrar una lista desplegable con los mapas del docente
 * @param integer $NumDocente N�mero de identificaci�n del docente
 * @param integer $TipoGrafica C�digo de la gr�fica seleccionada
 * @return xajaxResponse Objeto con la respuesta de la libreria XAjax
 */
function cargarMapasGrafica($NumDocente,$TipoGrafica){
	...
	$Conexion=abrirConexion();
	$Respuesta->addClear("SpanGrafica","innerHTML");
	$QueryMapas=mysql_query("SELECT id_mapa_conceptual as id, nombre_mapa as nom FROM mapa_conceptual WHERE usuario_id_usuario = '".$NumDocente."' order by id");
	$NumMapas=mysql_num_rows($QueryMapas);
	if($NumMapas>0){
...
/**
 * Esta funci�n se encarga de mostrar una lista desplegable con los estudiantes del docente segun grupo
 * @param integer $NumDocente N�mero de identificaci�n del docente
 * @param integer $TipoGrafica C�digo de la gr�fica seleccionada
 * @param integer $IdGrupo C�digo del grupo seleccionado
 * @return xajaxResponse Objeto con la respuesta de la libreria XAjax
 */
function cargarEstudiantes($NumDocente,$TipoGrafica,$IdGrupo){
	...
	if($IdGrupo==""){
		...
		$Conexion=abrirConexion();
		$QueryEst=mysql_query("SELECT id_usuario as id, nombre_usuario as nom, apellido_usuario as ape FROM usuario WHERE id_usuario IN (SELECT usuario_id_usuario FROM grupo_usuario WHERE grupo_id_grupo='".$IdGrupo."' AND usuario_id_usuario<>'".$NumDocente."') order by id");
		$NumEst=mysql_num_rows($QueryEst);
		if($NumEst>0){
...
/**
 * Esta funci�n se encarga de mostrar una lista desplegable con los mapas de un grupo seleccionado
 * @param integer $NumDocente N�mero de identificaci�n del docente
 * @param integer $TipoGrafica C�digo de la gr�fica seleccionada
 * @param integer $IdGrupo C�digo del grupo seleccionado
 * @param integer $IdEstudiante N�mero de identificaci�n del estudiante
 * @return xajaxResponse Objeto con la respuesta de la libreria XAjax
 */
function cargarMapas($NumDocente,$TipoGrafica,$IdGrupo,$IdEstudiante=""){
	...
	if($IdGrupo=="" && $TipoGrafica==6){
		...
		$Conexion=abrirConexion();
		$QueryMapas=mysql_query("SELECT id_mapa_conceptual as id, nombre_mapa as nom FROM mapa_conceptual WHERE id_mapa_conceptual IN (SELECT mapa_conceptual_id_mapa FROM grupo_mapa_conceptual WHERE grupo_id_grupo='".$IdGrupo."') order by id");
		$NumMapas=mysql_num_rows($QueryMapas);
		if($NumMapas>0){
...
/**
 * Esta funci�n se encarga de mostrar una lista desplegable con los tiempos para graficar resultados
 * @param integer $NumDocente N�mero de identificaci�n del docente
 * @param integer $TipoGrafica C�digo de la gr�fica seleccionada
 * @param integer $IdGrupo C�digo del grupo seleccionado
 * @param integer $IdEstudiante N�mero de identificaci�n del estudiante
...
/**
 * Esta funci�n se encarga de mostrar el grafico estadisticos segun los datosa proporcionados
 * @param integer $NumDocente N�mero de identificaci�n del docente
 * @param integer $TipoGrafica C�digo de la gr�fica seleccionada
 * @param integer $IdGrupo C�digo del grupo seleccionado
 * @param integer $IdMapa C�digo del mapa seleccionado
 * @param integer $IdEstudiante N�mero de identificaci�n del estudiante
 * @return xajaxResponse Objeto con la respuesta de la libreria XAjax
 */
function mostrarGrafica($NumDocente,$TipoGrafica,$IdGrupo,$IdMapa,$IdEstudiante=""){
	...
	switch($TipoGrafica){
		case 1:
			...
			//horizontal
			$Mapas=mysql_query("SELECT id_mapa_conceptual as id, nombre_mapa as nom FROM mapa_conceptual WHERE usuario_id_usuario='".$NumDocente."' ORDER BY id");
			if($Mapas){
				if(mysql_num_rows($Mapas)>0){
					...
					if($QueryGrupos){
						...
						if($NumGrupos>0){
							while($VecGrupos=mysql_fetch_array($QueryGrupos)){
								...
								$QueryMapas=mysql_query("SELECT id_mapa_conceptual as idmapa, nombre_mapa as mapa FROM mapa_conceptual WHERE id_mapa_conceptual IN (SELECT mapa_conceptual_id_mapa FROM grupo_mapa_conceptual WHERE grupo_id_grupo='".$VecGrupos["id"]."')");
								if($QueryMapas){
									$NumMapas=mysql_num_rows($QueryMapas);
									if($NumMapas>0){
										$ContMapas=0;
										while($VecMapas=mysql_fetch_array($QueryMapas)){
											...
											$QueryRes=mysql_query("SELECT respuestas_acertadas as rtas FROM historial_juego_respuesta WHERE juego_mapa_mapa_conceptual_id_mapa_conceptual='".$VecMapas["idmapa"]."' AND usuario_id_usuario IN (SELECT usuario_id_usuario FROM grupo_usuario WHERE grupo_id_grupo='".$VecGrupos["id"]."' AND usuario_id_usuario <> '".$NumDocente."')");
											if($QueryRes){
												$NumRtas=mysql_num_rows($QueryRes);
												if($NumRtas>0){
			...
			if($IdGrupo!=""){
				...
				if($Mapas){
					if(mysql_num_rows($Mapas)>0){
						...
						$ContGrupos=0;
						//primero debo buscar todos los grupos para medir su rendimiento
						$QueryGrupos=mysql_query("SELECT id_usuario as id, nombre_usuario as nom, apellido_usuario	as ape FROM usuario WHERE id_usuario IN (SELECT usuario_id_usuario FROM grupo_usuario WHERE grupo_id_grupo='".$IdGrupo."' AND usuario_id_usuario <> '".$NumDocente."') order by id");
						if($QueryGrupos){
							$NumGrupos=mysql_num_rows($QueryGrupos);
							if($NumGrupos>0){
								...
								while($VecGrupos=mysql_fetch_array($QueryGrupos)){
									...
									//ahora buscamos los mapas de ese grupo
									$QueryMapas=mysql_query("SELECT id_mapa_conceptual as idmapa, nombre_mapa as mapa FROM mapa_conceptual WHERE id_mapa_conceptual IN (SELECT mapa_conceptual_id_mapa FROM grupo_mapa_conceptual WHERE grupo_id_grupo='".$IdGrupo."')");
									if($QueryMapas){
										$NumMapas=mysql_num_rows($QueryMapas);
										if($NumMapas>0){
											$ContMapas=0;
											while($VecMapas=mysql_fetch_array($QueryMapas)){
												$Promedio=0;
												//ahora traemos los resultados de cada mapa de la tabla historial_juego_respuesta
												$QueryRes=mysql_query("SELECT respuestas_acertadas as rtas FROM historial_juego_respuesta WHERE juego_mapa_mapa_conceptual_id_mapa_conceptual='".$VecMapas["idmapa"]."' AND usuario_id_usuario='".$VecGrupos["id"]."'");
												if($QueryRes){
													$NumRtas=mysql_num_rows($QueryRes);
													if($NumRtas>0){
														while($VecRtas=mysql_fetch_array($QueryRes)){
		...
		case 3:
			if($IdMapa!=""){
				...
				$QueryGrupo=mysql_query("SELECT id_grupo as id, nombre_grupo as nom FROM grupo WHERE id_grupo IN (SELECT grupo_id_grupo FROM grupo_mapa_conceptual WHERE mapa_conceptual_id_mapa='".$IdMapa."' AND grupo_id_grupo IN (SELECT grupo_id_grupo FROM grupo_usuario WHERE usuario_id_usuario='".$NumDocente."'))");
				if($QueryGrupo){
					if(mysql_num_rows($QueryGrupo)>0){
						while($VecGrupo=mysql_fetch_array($QueryGrupo)){
							...
							$QueryEst=mysql_query("SELECT respuestas_acertadas as rtas FROM historial_juego_respuesta WHERE juego_mapa_mapa_conceptual_id_mapa_conceptual='".$IdMapa."' AND usuario_id_usuario IN (SELECT usuario_id_usuario FROM grupo_usuario WHERE grupo_id_grupo='".$VecGrupo["id"]."')");
							if($QueryEst){
								$NumRtas=mysql_num_rows($QueryEst);
								if($NumRtas>0){
									while($VecEst=mysql_fetch_array($QueryEst)){
						...
						$Respuesta->addAlert("Este mapa no se encuentra asociado con alg�n grupo. Por favor, dirijase a Mis Grupos y vinculelo con alg�n grupo disponible.");
						$Indicador=false;
					}
					if($Indicador!==false){
						//$Respuesta->addAlert(print_r($VecLineas,true));
		...
		case 4:
			$VecSecHor=array();
			$QueryMeses=mysql_query("SELECT distinct (to_char(fecha_realizacion,'YYYY-MM')) as fecha FROM historial_juego_respuesta WHERE usuario_id_usuario='".$IdEstudiante."' AND juego_mapa_mapa_conceptual_id_mapa_conceptual IN (SELECT mapa_conceptual_id_mapa FROM grupo_mapa_conceptual WHERE grupo_id_grupo='".$IdGrupo."')");
			if($QueryMeses){
				if(mysql_num_rows($QueryMeses)>0){
					while($VecMes=mysql_fetch_array($QueryMeses)){
						$Promedio=0;
						$Vm=explode("-",$VecMes["fecha"]);
						$VecSerieHorizontal[]=devolverMes($Vm[1])." - ".$Vm[0];
						$QueryRtas=mysql_query("SELECT respuestas_acertadas as rtas FROM historial_juego_respuesta WHERE usuario_id_usuario='".$IdEstudiante."' AND to_char(fecha_realizacion, 'YYYY-MM-DD') like '".$VecMes["fecha"]."-%'");
						$NumRtas=mysql_num_rows($QueryRtas);
						if($NumRtas>0){
							while($VecRtas=mysql_fetch_array($QueryRtas)){
								$vr=explode("/",$VecRtas["rtas"]);
								$Promedio+=floor(($vr[0]/$vr[1])*100);
							}
							$Promedio=$Promedio/$NumRtas;
...
/**
 * Esta funci�n se encarga de generar la imagen del grafico segun los datos proporcionados
 * @param integer $TipoGrafica C�digo de la gr�fica seleccionada
		...
		case 3:
			...
			// Initialise the graph
			 ...
			 $Test->drawGrid(4,TRUE,230,230,230,255);
			  ...
			  $Test->drawLegend(65,35,$DataSet->GetDataDescription(),255,255,255);
			  $Test->setFontProperties("lib/pChart.1.26e/Fonts/tahoma.ttf",10);
			  $Test->drawTitle(50,22,"Comparaci�n de grupos por mapa",50,50,50,585);
...
/**
 * Esta funci�n se encarga de mostrar un reporte estadistico segun los datos proporcionados
 * @param integer $NumDocente N�mero de identificaci�n del docente
 * @param integer $TipoGrafica C�digo de la gr�fica seleccionada
 * @param integer $IdGrupo C�digo del grupo seleccionado
 * @param integer $IdEstudiante N�mero de identificaci�n del estudiante
...
function mostrarTablaReportes($NumDocente,$TipoGrafica,$IdGrupo,$IdEstudiante="",$IdMapa){
	...
	if($TipoGrafica==5){
		...
		$QueryConceptos=mysql_query("SELECT concepto_id_concepto as padre, id_concepto_hijo as hijo, nombre_relacion as relacion FROM relacion WHERE concepto_mapa_conceptual_id_mapa_conceptual='".$IdMapa."' order by padre");
		if($QueryConceptos){
			if(mysql_num_rows($QueryConceptos)>0){
				while($VecConcepto=mysql_fetch_array($QueryConceptos)){
					$Promedio=0;
					$ContadorRtas=0;
					//traemos los nombres de los conceptos
					$VecPadre=mysql_fetch_array(mysql_query("SELECT nombre_concepto as nom FROM concepto WHERE id_concepto='".$VecConcepto["padre"]."' AND mapa_conceptual_id_mapa_conceptual = '".$IdMapa."'"));
					$VecHijo=mysql_fetch_array(mysql_query("SELECT nombre_concepto as nom FROM concepto WHERE id_concepto='".$VecConcepto["hijo"]."' AND mapa_conceptual_id_mapa_conceptual = '".$IdMapa."'"));
					...
					$QueryRta=mysql_query("SELECT valoracion_pregunta as rta FROM resultado_pregunta WHERE relacion_concepto_id_concepto='".$VecConcepto["padre"]."' AND relacion_id_concepto_hijo='".$VecConcepto["hijo"]."' AND relacion_concepto_mapa_conceptual_id_mapa_conceptual='".$IdMapa."' AND usuario_id_usuario IN (SELECT usuario_id_usuario FROM grupo_usuario WHERE grupo_id_grupo='".$IdGrupo."')");
					...
					if($QueryRta){
						$NumRtas=mysql_num_rows($QueryRta);
						//$Respuesta->addAlert($NumRtas);
						if($NumRtas>0){
							while($VecRta=mysql_fetch_array($QueryRta)){
				...
				<td colspan="2">
				<?php
					$VecTotales=mysql_fetch_array(mysql_query("select count(distinct r.usuario_id_usuario) as respreg, count(distinct gu.usuario_id_usuario) as grupousu from resultado_pregunta r, grupo_usuario gu WHERE gu.grupo_id_grupo='".$IdGrupo."' AND gu.usuario_id_usuario <> '".$NumDocente."' AND r.relacion_concepto_mapa_conceptual_id_mapa_conceptual='".$IdMapa."'"));
		...
		if($QueryConceptos){
			if(mysql_num_rows($QueryConceptos)>0){
				while($VecConcepto=mysql_fetch_array($QueryConceptos)){
					...
					$VecNombresConceptos[]=$VecPadre["nom"]." ".$VecConcepto["relacion"]." ".$VecHijo["nom"];
					//ahora se revisa la puntuacion en la tabla resultado_pregunta
					$QueryRta=mysql_query("SELECT valoracion_pregunta as rta FROM resultado_pregunta WHERE relacion_concepto_id_concepto='".$VecConcepto["padre"]."' AND relacion_concepto_mapa_conceptual_id_mapa_conceptual='".$IdMapa."' AND relacion_id_concepto_hijo='".$VecConcepto["hijo"]."' AND usuario_id_usuario = '".$IdEstudiante."'");
					//$Respuesta->addAlert("SELECT valoracion_pregunta as rta FROM resultado_pregunta WHERE relacion_concepto_id_concepto='".$VecConcepto["padre"]."' AND relacion_id_concepto_hijo='".$VecConcepto["hijo"]."' AND usuario_id_usuario IN (SELECT usuario_id_usuario FROM grupo_usuario WHERE grupo_id_grupo='".$IdGrupo."')");
					//como es de grupo hay que sacar del grupo el promedio
					if($QueryRta){
						$NumRtas=mysql_num_rows($QueryRta);
						//$Respuesta->addAlert($NumRtas);
						if($NumRtas>0){
							$VecRta=mysql_fetch_array($QueryRta);
							if($VecRta["rta"]=="t"){
								$ContadorRtas++;
							}
